...
for loaded_folder in loaded_folders
  ...
  Julz.include_all_files(loaded_folder, package_name="Plasmas")
...
function load_input(raw_input, is_file_input=false, can_be_missing=false, is_init_load=false)
  ...
  old_symbols = Julz.get_all_symbols(Plasmas)
  eval( parse(raw_input) )
  new_symbols = setdiff(Julz.get_all_symbols(Plasmas), old_symbols)

  is_init_load && ( new_symbols = [] )

  for cur_symbol in setdiff(new_symbols, old_symbols)
    cur_expression = quote
      $(cur_symbol) = $(getfield(Plasmas, cur_symbol))
    end
...
print_workspace() = Julz.print_workspace(Plasmas)
